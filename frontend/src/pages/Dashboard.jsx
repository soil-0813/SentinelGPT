...
import { fetchThreatIntel } from "../services/api.js";
...
export default function Dashboard() {
  ...
  async function load() {
    ...
    const [s, a, inc, tl, intel] = await Promise.all([
      fetchDashboardStats(),
      fetchAlerts(),
      fetchIncidents(),
      fetchTimeline(),
  ...
  return (
    <div className="page-body">
      ...
      <div className="stat-grid">
        {STAT_CARDS.map((c) => (
          ...
          >
            ...
            <div className="stat-value" style={{ color: c.accentColor }}>
              {loading ? "—" : c.value}
            </div>
      ...
      <div
        ...
        style={{ gridTemplateColumns: "2fr 1fr", marginBottom: 20 }}
      >
        {/* Activity Area Chart */}
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart
              data={stats?.activity?.length ? stats.activity : ACTIVITY_DATA}
      ...
      >
        ...
        >
          <div className="card-header">
            ...
            <span className="card-badge cyan">{timeline.length} events</span>
          </div>
        ...
        >
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            <ThreatIntelPanel items={threatIntel} />
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="card-header">
          ...
          >
            {incidents.flatMap((i) => i.mitre ?? []).filter(Boolean).length}{" "}
            techniques
          </span>
        </div>
        <MitreMapping
          highlighted={incidents.flatMap((i) => i.mitre ?? []).filter(Boolean)}
        />
      </div>
